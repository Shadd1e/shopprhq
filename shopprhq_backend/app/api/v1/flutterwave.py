...
async def flutterwave_webhook(request: Request):
    ...
    # Read secret at request time so tests can patch it via environment
    secret = os.getenv("FLUTTERWAVE_WEBHOOK_SECRET") or FLUTTERWAVE_WEBHOOK_SECRET
    signature = request.headers.get("verif-hash")

    # Log immediately so we can see every hit regardless of what happens next
    logger.info(
        "Flutterwave webhook hit — signature_present=%s secret_configured=%s",
    ...
    if not secret:
        logger.error("Flutterwave webhook: FLUTTERWAVE_WEBHOOK_SECRET not configured in env")
        raise HTTPException(status_code=500, detail="Webhook secret not configured")

    # Ensure both are str — compare_digest raises TypeError if types differ
    try:
        sig_str = signature if isinstance(signature, str) else signature.decode()
        sec_str = secret if isinstance(secret, str) else secret.decode()
        match = hmac.compare_digest(sig_str, sec_str)
    except Exception as e:
        logger.error("Flutterwave webhook: signature comparison error: %s", e)
        raise HTTPException(status_code=500, detail="Signature comparison failed")

    if not match:
        logger.error("Flutterwave webhook: signature mismatch — header present=%s", bool(signature))
    ...
    logger.info(
        "Flutterwave charge.completed — status=%s tx_ref=%s tx_id=%s",
        status, tx_ref, tx_id,
    )
    ...
    dedup_key = f"flw_processed:{tx_id}"

    try:
        _rc = await redis_service.get_client()
        already_seen = await _rc.get(dedup_key)
        if already_seen:
            logger.info("Duplicate Flutterwave webhook ignored: %s", tx_id)
    ...
    async with AsyncSessionLocal() as db:
        async with db.begin():
            ...
            if not payment or payment.status != PaymentStatus.SUCCEEDED:
                logger.error("Flutterwave webhook: payment not succeeded for tx_ref=%s", tx_ref)
                try:
                    ...
                    fire_and_forget(alert(
                        ...
                        level="critical",
                        fields={"tx_ref": tx_ref, "status": status},
                    ), name="alert_flw_payment_failed")
                except Exception:
                    pass
            ...
            if not order:
                logger.error("Flutterwave webhook: order not found for payment %s", payment.id)
                try:
                    from app.infrastructure.alerting.slack import alert
                    from app.api.v1.workers.background_tasks import fire_and_forget
                    fire_and_forget(alert(
                    ...
                    ), name="alert_flw_order_missing")
                except Exception:
                    pass
                return {"status": "order_missing"}

            if order.status == OrderStatus.PAID:
                logger.info("Flutterwave webhook: order %s already paid", order.id)
            ...
            if not cart:
                logger.error("Flutterwave webhook: cart not found for order %s", order.id)
                try:
                    from app.infrastructure.alerting.slack import alert
                    from app.api.v1.workers.background_tasks import fire_and_forget
                    fire_and_forget(alert(
    ...
    try:
        _rc = await redis_service.get_client()
        await _rc.setex(dedup_key, 86400, "1")
    except Exception as e:
